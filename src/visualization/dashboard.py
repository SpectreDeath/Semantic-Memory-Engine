...
class RhetoricAnalyzer:
    ...
    def analyze_moral_foundations(self, df: pd.DataFrame) -> dict[str, float]:
        ...
        foundations = {
            ...
            "Fairness/Cheating": {
                "terms": ["fair", "unfair", "just", "cheat", "dishonest"],
                "intensity": 0,
            },
            "Loyalty/Betrayal": {
                "terms": ["loyal", "betray", "traitor", "ally", "enemy"],
                "intensity": 0,
            },
            "Authority/Subversion": {
                "terms": ["rule", "law", "respect", "rebel", "authority"],
                "intensity": 0,
            },
            "Sanctity/Degradation": {
                "terms": ["pure", "degraded", "sacred", "corrupt", "contaminate"],
                "intensity": 0,
            },
            "Liberty/Oppression": {
                "terms": ["freedom", "liberty", "oppression", "chains", "dominate"],
                "intensity": 0,
            },
        }
    ...
    def predict_escalation(self, df: pd.DataFrame, window: int = 7) -> dict[str, Any]:
        ...
        return {
            ...
            if trend_strength > 0.2 and trend_direction == "escalating"
            else "normal",
        }
...
def main():
    ...
    st.markdown(
        "Real-time visualization of moral foundation trends and rhetoric escalation alerts."
    )
    ...
    with st.sidebar:
        ...
        analysis_mode = st.radio(
            "Analysis Mode", ["Trends", "Pharos (Predictive)", "Foundations", "Alerts"]
        )
    ...
    if analysis_mode == "Trends":
        ...
        with col2:
            ...
            st.metric(
                "Trend",
                "📈 Improving"
                if df["compound"].tail(7).mean() > df["compound"].iloc[-14:-7].mean()
                else "📉 Declining",
            )
    ...
    # 2. PHAROS (PREDICTIVE) TAB
        ...
        if prediction["status"] != "insufficient_data":
            ...
            if len(df) > 14:
                ...
                future_dates = [
                    df["timestamp"].iloc[-1] + timedelta(days=i + 1) for i in range(future_steps)
                ]
                fig.add_trace(
                    go.Scatter(
                        x=future_dates,
    ...
    # 4. ALERTS TAB
        ...
        alert_level = (
            "🔴 CRITICAL" if prediction.get("alert") == "escalation_detected" else "🟢 NORMAL"
        )
    ...
    st.markdown("---")
    st.caption(
        "🔭 Beacon Dashboard | SimpleMem Rhetoric Monitoring | Last updated: "
