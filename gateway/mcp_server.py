...
from gateway.tool_registry import (
    get_registry, ToolRegistry, ToolDefinition, 
    ScribeAuthorshipTool, ScribeProTool, InfluenceTool, 
    EpistemicValidator, ForensicMathTool, ForensicFilesTool,
    ForensicBehaviorTool, ForensicGraphTool, ForensicSignalTool,
    ForensicEntropyTool, ForensicCrawlerTool, ForensicScorerTool
)
...
@mcp.tool()
def calculate_tfidf(tokenized_docs: List[List[str]], session_id: Optional[str] = None) -> str:
    """Calculate term significance (TF-IDF) across a corpus of documents."""
    math_tool.core.session_id = session_id
    result = math_tool.calculate_tfidf(tokenized_docs)
    return json.dumps(result, indent=2)


@mcp.tool()
def calculate_kl_divergence(p: List[float], q: List[float], session_id: Optional[str] = None) -> str:
    """Measure relative entropy (KL Divergence) between two distributions."""
    math_tool.core.session_id = session_id
    result = math_tool.calculate_kl_divergence(p, q)
    return json.dumps(result, indent=2)
...
def calculate_phonetic_hash(word: str, session_id: Optional[str] = None) -> str:
    ...
    math_tool.core.session_id = session_id
    result = math_tool.calculate_phonetic_hash(word)
    return json.dumps(result, indent=2)


@mcp.tool()
def audit_benford_distribution(data: List[float], session_id: Optional[str] = None) -> str:
    """Fraud detection using Benford's Law distribution analysis."""
    math_tool.core.session_id = session_id
    result = math_tool.audit_benford_distribution(data)
    return json.dumps(result, indent=2)


@mcp.tool()
def calculate_simhash(tokens: List[str], hash_size: int = 64, session_id: Optional[str] = None) -> str:
    """Locality Sensitive Hashing (SimHash) for near-duplicate detection."""
    math_tool.core.session_id = session_id
    result = math_tool.calculate_simhash(tokens, hash_size)
    return json.dumps(result, indent=2)


@mcp.tool()
def calculate_entropy_divergence(p: List[float], q: List[float], session_id: Optional[str] = None) -> str:
    """Measure relative entropy (Data Drift) between two distributions."""
    math_tool.core.session_id = session_id
    result = math_tool.calculate_entropy_divergence(p, q)
    return json.dumps(result, indent=2)
...
def verify_file_signature(file_path: str, session_id: Optional[str] = None) -> str:
    ...
    result = files_tool.verify_file_signature(file_path)
    return json.dumps(result, indent=2)


@mcp.tool()
def calculate_structural_complexity(file_path: str, session_id: Optional[str] = None) -> str:
    """Calculate compression ratio as structural entropy proxy."""
    files_tool.core.session_id = session_id
    result = files_tool.calculate_structural_complexity(file_path)
    return json.dumps(result, indent=2)


@mcp.tool()
def calculate_burstiness(timestamps: List[float], session_id: Optional[str] = None) -> str:
    """Calculate temporal burstiness score."""
    behavior_tool.core.session_id = session_id
    result = behavior_tool.calculate_burstiness(timestamps)
    return json.dumps(result, indent=2)


@mcp.tool()
def validate_luhn_checksum(numeric_string: str, session_id: Optional[str] = None) -> str:
    """Validate numeric leakage via Luhn algorithm."""
    behavior_tool.core.session_id = session_id
    result = behavior_tool.validate_luhn_checksum(numeric_string)
    return json.dumps(result, indent=2)


@mcp.tool()
def calculate_node_path(graph: dict, start_node: str, end_node: str, session_id: Optional[str] = None) -> str:
    """Find the shortest path between nodes in a relationship graph."""
    graph_tool.core.session_id = session_id
    result = graph_tool.calculate_node_path(graph, start_node, end_node)
    return json.dumps(result, indent=2)


@mcp.tool()
def identify_central_hubs(adjacency_matrix: List[List[float]], node_labels: List[str], session_id: Optional[str] = None) -> str:
    """Identify influential nodes using Eigenvector Centrality."""
    graph_tool.core.session_id = session_id
    result = graph_tool.identify_central_hubs(adjacency_matrix, node_labels)
    return json.dumps(result, indent=2)


@mcp.tool()
def calculate_sequence_similarity(seq1: List[float], seq2: List[float], session_id: Optional[str] = None) -> str:
    """Find similarity between time-series sequences using DTW."""
    signal_tool.core.session_id = session_id
    result = signal_tool.calculate_sequence_similarity(seq1, seq2)
    return json.dumps(result, indent=2)


@mcp.tool()
def detect_event_periodicity(data: List[float], session_id: Optional[str] = None) -> str:
    """Identify dominant periodicities via DFT."""
    signal_tool.core.session_id = session_id
    result = signal_tool.detect_event_periodicity(data)
    return json.dumps(result, indent=2)


@mcp.tool()
def map_stream_entropy(stream: List[int], window_size: int = 256, session_id: Optional[str] = None) -> str:
    """Map Shannon entropy across a byte-stream using a sliding window."""
    entropy_tool.core.session_id = session_id
    result = entropy_tool.map_stream_entropy(stream, window_size)
    return json.dumps(result, indent=2)


@mcp.tool()
def analyze_obfuscation_score(content: str, session_id: Optional[str] = None) -> str:
    """Detect obfuscated scripts using Hamming Weight and Compression complexity."""
    entropy_tool.core.session_id = session_id
    result = entropy_tool.analyze_obfuscation_score(content)
    return json.dumps(result, indent=2)
...
def get_forensic_report(target_text: str, session_id: Optional[str] = None) -> str:
    ...
    result = scorer_tool.get_forensic_report(target_text)
    return json.dumps(result, indent=2)
