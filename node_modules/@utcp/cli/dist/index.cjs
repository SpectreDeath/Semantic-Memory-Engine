...
__export(index_exports, {
  CliCallTemplateSchema: () => CliCallTemplateSchema,
  CliCallTemplateSerializer: () => CliCallTemplateSerializer,
  CliCommunicationProtocol: () => CliCommunicationProtocol,
  CommandStepSchema: () => CommandStepSchema,
  register: () => register
});
...
// src/cli_call_template.ts
var import_sdk = require("@utcp/sdk");
var CommandStepSchema = external_exports.object({
  command: external_exports.string().describe("Command string to execute, may contain UTCP_ARG_argname_UTCP_END placeholders"),
  append_to_final_output: external_exports.boolean().nullable().optional().describe("Whether to include this command's output in final result. Defaults to False for all except last command")
}).strict();
var CliCallTemplateSchema = external_exports.object({
  name: external_exports.string().optional(),
  call_template_type: external_exports.literal("cli"),
  commands: external_exports.array(CommandStepSchema).describe("List of commands to execute in order. Each command can contain UTCP_ARG_argname_UTCP_END placeholders."),
  env_vars: external_exports.record(external_exports.string(), external_exports.string()).nullable().optional().describe("Environment variables to set when executing the commands"),
  working_dir: external_exports.string().nullable().optional().describe("Working directory for command execution"),
  auth: external_exports.undefined().optional(),
...
var CliCallTemplateSerializer = class extends import_sdk.Serializer {
  /**
   * REQUIRED
...
// src/cli_communication_protocol.ts
var import_sdk2 = require("@utcp/sdk");
var import_child_process = require("child_process");
var import_timers = require("timers");
var CliCommunicationProtocol = class {
  /**
  ...
  async _executeShellScript(script, options = {}, timeoutMs = 6e4) {
    ...
    try {
      ...
      childProcess = (0, import_child_process.spawn)(shell, args, {
        cwd: options.cwd,
        env: mergedEnv,
      ...
      const timeoutPromise = new Promise((_, reject) => {
        ...
        childProcess?.on("close", () => (0, import_timers.clearTimeout)(id));
      });
   ...
   */
  _substitute_utcp_args(command, toolArgs) {
   ...
   */
  _build_combined_shell_script(commands, toolArgs) {
    const isWindows = process.platform === "win32";
    const scriptLines = [];
    if (isWindows) {
    ...
    for (let i = 0; i < commands.length; i++) {
      ...
      if (shouldAppend === null || shouldAppend === void 0) {
  ...
  async registerManual(caller, manualCallTemplate) {
    if (!manualCallTemplate.commands || manualCallTemplate.commands.length === 0) {
    ...
    try {
      ...
      const { stdout, stderr, exitCode } = await this._executeShellScript(shellScript, {
        cwd: cliCallTemplate.working_dir || void 0,
        env
      }, 3e4);
      ...
      if (!output.trim()) {
        ...
        return {
          success: false,
          manualCallTemplate,
          manual: new import_sdk2.UtcpManualSerializer().validateDict({ tools: [] }),
          errors: [`No output from discovery commands for CLI provider '${manualCallTemplate.name}'`]
        };
      }
      try {
        const utcpManual = import_sdk2.UtcpManualSchema.parse(JSON.parse(output.trim()));
  ...
  async callTool(caller, toolName, toolArgs, toolCallTemplate) {
    if (!toolCallTemplate.commands || toolCallTemplate.commands.length === 0) {
...
function register(override = false) {
  import_sdk3.CallTemplateSerializer.registerCallTemplate("cli", new CliCallTemplateSerializer(), override);
  import_sdk3.CommunicationProtocol.communicationProtocols["cli"] = new CliCommunicationProtocol();
}
register();
// Annotate the CommonJS export names for ESM import in node:
0 && (module.exports = {
  CliCallTemplateSchema,
