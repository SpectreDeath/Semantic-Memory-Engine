...
def generate_session_report(output_path, osint_data, news_data, research_data):
    ...
    pdf.multi_cell(
        0,
        10,
        f"This report summarizes the findings from the current SME intelligence gathering session. "
...
def generate_case_report(output_path, osint_data, sentiment_data=None):
            ...
            if logs:
                ...
                for entry in logs[-5:]:  # Show last 5
                    pdf.cell(
                        0,
                        7,
                        f"[{entry['timestamp'][:16]}] Target: {entry['target']} ({entry['action']})",
                        0,
                        1,
                    )
                pdf.ln(5)
    ...
    if sentiment_data is not None and not sentiment_data.empty:
        ...
        pdf.multi_cell(
            0,
            10,
            f"Average session sentiment polarity: {avg_pol:.2f}. "
