...
def vendor_faststylometry():
    base_url = (
        "https://raw.githubusercontent.com/fastdatascience/faststylometry/main/src/faststylometry/"
    )
    ...
    for filename in files:
        ...
        if response.status_code == 200:
            ...
            patched_content = re.sub(
                r"from faststylometry(\..+)? import", r"from .\1 import", content
            )
            patched_content = re.sub(r"\.\.(\.)?", r".", patched_content)  # Fix .. to .
            patched_content = re.sub(
                r"import faststylometry",
