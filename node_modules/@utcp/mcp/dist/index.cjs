...
__export(index_exports, {
  McpCallTemplateSchema: () => McpCallTemplateSchema,
  McpCallTemplateSerializer: () => McpCallTemplateSerializer,
  McpCommunicationProtocol: () => McpCommunicationProtocol,
  McpConfigSchema: () => McpConfigSchema,
  McpHttpServerSchema: () => McpHttpServerSchema,
  McpServerConfigSchema: () => McpServerConfigSchema,
  McpStdioServerSchema: () => McpStdioServerSchema,
  register: () => register
});
...
// src/mcp_call_template.ts
var import_sdk = require("@utcp/sdk");
var import_sdk2 = require("@utcp/sdk");
var McpStdioServerSchema = external_exports.object({
  transport: external_exports.literal("stdio"),
  command: external_exports.string().describe("The command to execute the MCP server."),
  args: external_exports.array(external_exports.string()).optional().default([]).describe("Arguments to pass to the command."),
  cwd: external_exports.string().optional().describe("Working directory for the command."),
  env: external_exports.record(external_exports.string(), external_exports.string()).optional().default({}).describe("Environment variables for the command."),
  timeout: external_exports.number().optional().default(30).describe("Timeout for MCP operations in seconds.")
});
var McpHttpServerSchema = external_exports.object({
  transport: external_exports.literal("http"),
  url: external_exports.string().describe("The URL of the MCP HTTP server endpoint."),
  headers: external_exports.record(external_exports.string(), external_exports.string()).optional().describe("Optional HTTP headers for the connection."),
  timeout: external_exports.number().optional().default(30).describe("Timeout for HTTP requests in seconds."),
  sse_read_timeout: external_exports.number().optional().default(300).describe("Read timeout for SSE connections in seconds (e.g., for `streamable-http` MCP servers)."),
  terminate_on_close: external_exports.boolean().optional().default(true).describe("Whether to terminate the HTTP connection on client close.")
});
var McpServerConfigSchema = external_exports.discriminatedUnion("transport", [
  McpStdioServerSchema,
  McpHttpServerSchema
]);
var McpConfigSchema = external_exports.object({
  mcpServers: external_exports.record(external_exports.string(), external_exports.any()).describe("Dictionary mapping server names to their configurations.")
});
var McpCallTemplateSchema = external_exports.object({
  name: external_exports.string().optional(),
  call_template_type: external_exports.literal("mcp"),
  ...
  auth: import_sdk.AuthSchema.nullable().optional().describe("Optional OAuth2 authentication for HTTP-based MCP servers."),
  register_resources_as_tools: external_exports.boolean().default(false).describe("Whether to register MCP resources as callable tools. When True, server resources are exposed as tools that can be called."),
...
var McpCallTemplateSerializer = class extends import_sdk2.Serializer {
  /**
   * REQUIRED
...
// src/mcp_communication_protocol.ts
...
function isMcpToolsResponse(data) {
  return typeof data === "object" && data !== null && "tools" in data && Array.isArray(data.tools);
}
var McpCommunicationProtocol = class {
  _oauthTokens = /* @__PURE__ */ new Map();
  _axiosInstance;
  _mcpSessions = /* @__PURE__ */ new Map();
  constructor() {
    this._axiosInstance = import_axios.default.create({ timeout: 3e4 });
  }
  _logInfo(message) {
   ...
   */
  async _dereferenceSchema(schema) {
    ...
    try {
      ...
      const dereferenced = await import_json_schema_ref_parser.default.dereference(schema);
      return dereferenced;
    } catch (error) {
  ...
  async _getOrCreateSession(serverName, serverConfig, auth) {
    ...
    if (serverConfig.transport === "stdio") {
      ...
      transport = new import_streamableHttp.StreamableHTTPClientTransport(new URL(httpConfig.url), transportOptions);
    } else {
      throw new Error(`Unsupported MCP transport: '${serverConfig.transport}'`);
    }
    const mcpClient = new import_client.Client({ name: `utcp-mcp-client-${sessionKey}`, version: "1.0.1" });
    try {
  ...
  async _withSession(serverName, serverConfig, auth, operation) {
    const sessionKey = `${serverName}:${serverConfig.transport}`;
    const timeoutMs = (serverConfig.timeout ?? 30) * 1e3;
    try {
      const client = await this._getOrCreateSession(serverName, serverConfig, auth);
      return await Promise.race([
        operation(client),
        new Promise((_, reject) => setTimeout(() => reject(new Error(`MCP operation on '${sessionKey}' timed out after ${timeoutMs / 1e3}s.`)), timeoutMs))
      ]);
    } catch (e) {
      this._logError(`MCP operation on '${sessionKey}' failed:`, e.message);
      const errorMsg = String(e?.message ?? e).toLowerCase();
      if (errorMsg.includes("closed") || errorMsg.includes("disconnected") || errorMsg.includes("econnreset") || errorMsg.includes("etimedout") || errorMsg.includes("already initialized")) {
        ...
        return await Promise.race([operation(newClient), new Promise((_, reject) => setTimeout(() => reject(new Error(`MCP operation on '${sessionKey}' timed out after ${timeoutMs / 1e3}s.`)), timeoutMs))]);
      }
      throw e;
    }
  }
  async registerManual(caller, manualCallTemplate) {
    ...
    if (!mcpCallTemplate.config?.mcpServers || Object.keys(mcpCallTemplate.config.mcpServers).length === 0) {
      ...
      return { manualCallTemplate: mcpCallTemplate, manual: import_sdk3.UtcpManualSchema.parse({ tools: [] }), success: false, errors: [errorMsg] };
    }
    const allTools = [];
    const allErrors = [];
    for (const [serverName, serverConfig] of Object.entries(mcpCallTemplate.config.mcpServers)) {
      try {
        ...
        const mcpToolsResult = await this._withSession(
          serverName,
          serverConfig,
          mcpCallTemplate.auth,
          (client) => client.listTools()
        );
        if (!isMcpToolsResponse(mcpToolsResult)) {
          throw new Error("Invalid response format from listTools");
        }
        const utcpToolsPromises = mcpToolsResult.tools.map(async (mcpTool) => {
    ...
    return {
      ...
      manual: import_sdk3.UtcpManualSchema.parse({ tools: allTools }),
  ...
  async callTool(caller, toolName, toolArgs, toolCallTemplate) {
    ...
    const result = await this._withSession(
      serverName,
      serverConfig,
      mcpCallTemplate.auth,
    ...
    try {
      const token = await Promise.any([
        (async () => {
          ...
          const expiresAt = Date.now() + (response.data.expires_in || 3600) * 1e3;
      ...
      const errorMessages = aggregateError.errors?.map((e) => e.message).join("; ") || String(aggregateError);
...
function register(override = false) {
  import_sdk4.CallTemplateSerializer.registerCallTemplate("mcp", new McpCallTemplateSerializer(), override);
  import_sdk4.CommunicationProtocol.communicationProtocols["mcp"] = new McpCommunicationProtocol();
}
register();
// Annotate the CommonJS export names for ESM import in node:
0 && (module.exports = {
  McpCallTemplateSchema,
