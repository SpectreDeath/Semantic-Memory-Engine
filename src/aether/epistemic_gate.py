...
class GateStats:
    ...
    def from_dict(cls, data: dict[str, Any]) -> GateStats:
        return cls(
            ...
            last_training=datetime.fromisoformat(data["last_training"])
...
class EpistemicGate:
    ...
    def _combine_features(
        self, numeric_features: np.ndarray, text_features: np.ndarray
    ) -> np.ndarray:
        """Combine numeric and text features."""
        # Limit text features to save memory
        text_features_limited = (
            text_features[:, :100] if text_features.shape[1] > 100 else text_features
        )
    ...
    def evaluate(self, text: str, return_features: bool = False) -> dict[str, Any]:
        """
        ...
        with self._lock:
            ...
            final_decision = self._combine_decisions(
                base_decision, base_label, model_decision, model_confidence
            )
    ...
    def _combine_decisions(
        self, base_score: float, base_label: str, model_pred: str | None, model_conf: float | None
    ) -> dict[str, Any]:
        """
        ...
        if consensus:
            ...
            self._log_potential_failure(
                base_verdict, model_pred, max(base_score / 100.0, model_conf)
            )

        return {"decision": decision, "confidence": confidence, "reasons": reasons}

    def _log_potential_failure(
        self, base_verdict: str, model_verdict: str, confidence: float
    ) -> None:
        """Log potential failure for later analysis."""
    ...
    def report_feedback(
        self, text: str, original_decision: str, corrected_label: str, notes: str = ""
    ) -> str:
        """
        ...
        with self._lock:
            ...
            if self.auto_train:
                total_failures = self._stats.false_positives + self._stats.false_negatives
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        if total > 0:
            stats["precision"] = self._stats.true_positives / max(
                1, self._stats.true_positives + self._stats.false_positives
            )
            stats["recall"] = self._stats.true_positives / max(
                1, self._stats.true_positives + self._stats.false_negatives
            )
    ...
    def get_recent_failures(self, limit: int = 10) -> list[dict[str, Any]]:
        ...
        return [self._failures[fid].to_dict() for fid in recent_ids if fid in self._failures]
