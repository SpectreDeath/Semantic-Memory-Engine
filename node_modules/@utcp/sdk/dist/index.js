...
var CallTemplateSchema = z.custom((obj) => {
  try {
...
var JsonTypeSchema = z2.lazy(() => z2.union([
  z2.string(),
  z2.number(),
  z2.boolean(),
  z2.null(),
  z2.record(z2.string(), JsonTypeSchema),
  z2.array(JsonTypeSchema)
]));
var JsonSchemaSchema = z2.lazy(() => z2.object({
  $schema: z2.string().optional().describe("JSON Schema version URI."),
  $id: z2.string().optional().describe("A URI for the schema."),
  title: z2.string().optional().describe("A short explanation about the purpose of the data described by this schema."),
  description: z2.string().optional().describe("A more lengthy explanation about the purpose of the data described by this schema."),
  type: z2.union([
    z2.literal("string"),
    z2.literal("number"),
    z2.literal("integer"),
  ...
  properties: z2.record(z2.string(), z2.lazy(() => JsonSchemaSchema)).optional(),
  items: z2.union([z2.lazy(() => JsonSchemaSchema), z2.array(z2.lazy(() => JsonSchemaSchema))]).optional(),
  required: z2.array(z2.string()).optional(),
  enum: z2.array(JsonTypeSchema).optional(),
  const: JsonTypeSchema.optional(),
  default: JsonTypeSchema.optional(),
  format: z2.string().optional(),
  additionalProperties: z2.union([z2.boolean(), z2.lazy(() => JsonSchemaSchema)]).optional(),
...
var ToolSchema = z2.object({
  name: z2.string().describe('Unique identifier for the tool, typically in format "manual_name.tool_name".'),
  description: z2.string().default("").describe("Human-readable description of what the tool does."),
  ...
  tags: z2.array(z2.string()).default([]).describe("List of tags for categorization and search."),
  average_response_size: z2.number().optional().describe("Optional hint about typical response size in bytes."),
...
var UtcpManualSchema = z3.object({
  // Use .optional() to allow missing in input, then .default() to satisfy the interface.
  utcp_version: z3.string().optional().default(UTCP_PACKAGE_VERSION).describe("UTCP protocol version supported by the provider."),
  manual_version: z3.string().optional().default("1.0.0").describe("Version of this specific manual."),
  tools: z3.array(ToolSchema).describe("List of available tools with their complete configurations.")
}).strict();
...
var ApiKeyAuthSchema = z5.object({
  auth_type: z5.literal("api_key"),
  api_key: z5.string(),
  var_name: z5.string().default("X-Api-Key"),
  location: z5.enum(["header", "query", "cookie"]).default("header")
});
...
var BasicAuthSchema = z6.object({
  auth_type: z6.literal("basic"),
  username: z6.string().describe("The username for basic authentication. Recommended to use injected variables."),
  password: z6.string().describe("The password for basic authentication. Recommended to use injected variables.")
}).strict();
...
var OAuth2AuthSchema = z7.object({
  auth_type: z7.literal("oauth2"),
  token_url: z7.string().describe("The URL to fetch the OAuth2 access token from. Recommended to use injected variables."),
  client_id: z7.string().describe("The OAuth2 client ID. Recommended to use injected variables."),
  client_secret: z7.string().describe("The OAuth2 client secret. Recommended to use injected variables."),
  scope: z7.string().optional().describe("Optional scope parameter to limit the access token's permissions.")
}).strict();
...
var InMemConcurrentToolRepositoryConfigSchema = z8.object({
  tool_repository_type: z8.literal("in_memory")
}).passthrough();
...
var ConcurrentToolRepositorySchema = z9.custom((obj) => {
  try {
...
var TagSearchStrategyConfigSchema = z10.object({
  tool_search_strategy_type: z10.literal("tag_and_description_word_match"),
  description_weight: z10.number().optional().default(1),
  tag_weight: z10.number().optional().default(3)
}).passthrough();
...
var ToolSearchStrategySchema = z11.custom((obj) => {
  try {
...
var FilterDictPostProcessorConfigSchema = z12.object({
  tool_post_processor_type: z12.literal("filter_dict"),
  exclude_keys: z12.array(z12.string()).optional(),
  ...
  only_include_manuals: z12.array(z12.string()).optional()
}).passthrough();
...
var LimitStringsPostProcessorConfigSchema = z13.object({
  tool_post_processor_type: z13.literal("limit_strings"),
  limit: z13.number().int().positive().default(1e4),
  exclude_tools: z13.array(z13.string()).optional(),
  only_include_tools: z13.array(z13.string()).optional(),
  exclude_manuals: z13.array(z13.string()).optional(),
  only_include_manuals: z13.array(z13.string()).optional()
}).passthrough();
...
var ToolPostProcessorSchema = z14.custom((obj) => {
  try {
...
var VariableLoaderSchema = z15.custom((obj) => {
  try {
...
var UtcpClientConfigSchema = z16.object({
  variables: z16.record(z16.string(), z16.string()).optional().default({}),
  load_variables_from: z16.array(VariableLoaderSchema).nullable().optional().default(null).transform((val) => {
  ...
  tool_repository: z16.any().transform((val) => {
  ...
  tool_search_strategy: z16.any().transform((val) => {
  ...
  post_processing: z16.array(z16.any()).transform((val) => {
    return val.map((item) => {
  ...
  manual_call_templates: z16.array(CallTemplateSchema).transform((val) => {
    return val.map((item) => {
