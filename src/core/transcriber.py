    ...
    def transcribe_youtube(self, url: str) -> dict[str, Any]:
        ...
        try:
            ...
            transcript_file = os.path.join(
                TRANSCRIPTS_DIR, f"transcript_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
            ...
            return {
                ...
                "transcript": transcribe_result["text"][:500] + "..."
                if len(transcribe_result["text"]) > 500
                else transcribe_result["text"],
            }
...
def check_transcription_dependencies() -> str:
    ...
    dependencies = {
        ...
        "ffmpeg": {"note": "Required by yt-dlp. Download from https://ffmpeg.org/download.html"},
    }
