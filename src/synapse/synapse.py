    ...
    def consolidate_idle(self, min_cluster_size: int = 3) -> dict[str, Any]:
        ...
        for i, cluster in enumerate(clusters):
            if len(cluster) >= min_cluster_size:
                ...
                concept_name = (
                    f"profile_{i}_{hashlib.md5(''.join(cluster).encode()).hexdigest()[:6]}"
                )
...
def find_similar_memories(similarity_threshold: float = 0.6) -> str:
    ...
    try:
        ...
        result = {
            ...
            "clusters": [{"size": len(cluster), "members": cluster} for cluster in clusters],
            "status": "success",
        }
...
def build_behavioral_profile(entity_name: str, days: int = 30) -> str:
    ...
    try:
        ...
        return json.dumps({"behavioral_profile": profile, "status": "success"}, indent=2)
