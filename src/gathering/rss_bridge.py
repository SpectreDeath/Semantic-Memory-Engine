...
def main():
    ...
    parser.add_argument(
        "--hours", type=int, default=24, help="Limit to entries from the last X hours (default: 24)"
    )
    ...
    if all_entries:
        ...
        print(
            f"✅ Successfully ingested {len(all_entries)} new entries (last {args.hours}h) to {path}"
        )
    else:
