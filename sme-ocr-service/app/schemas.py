...
class OcrBlock(BaseModel):
    ...
    bbox: list[float] = Field(
        description="Bounding box [x1, y1, x2, y2]", examples=[[10.0, 20.0, 100.0, 80.0]]
    )
    text: str = Field(description="Extracted text from the block")
    confidence: float = Field(description="OCR confidence score (0-1)", ge=0.0, le=1.0)
...
class OcrPage(BaseModel):
    ...
    dimensions: dict[str, int] | None = Field(
        default=None, description="Page dimensions {width, height} in points"
    )
...
class OcrMeta(BaseModel):
    ...
    content_type: str = Field(description="MIME type (application/pdf, image/png, etc.)")
...
class HealthResponse(BaseModel):
    ...
    engine_version: str | None = Field(default=None, description="PaddleOCR version if available")
