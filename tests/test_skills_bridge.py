...
from gateway.skills_bridge import _make_skill_handler, register_skills_with_registry
...
def test_make_skill_handler_search():
    ...
    mock_skill1.name = "SkillA"
    mock_skill1.category = "catA"
    mock_skill1.purpose = "Purpose A"
    mock_skill2 = MagicMock()
    mock_skill2.name = "SkillB"
    mock_skill2.category = "catB"
    mock_skill2.purpose = "Purpose B"
    ...
    assert result["results"][1]["name"] == "SkillB"
...
@patch("src.sme.skills_loader.SkillsLoader")
def test_register_skills_with_registry_calls_add_tool(mock_loader_class):
    ...
    mock_skill.category = "test_category"
    mock_skill.purpose = "Test skill purpose"
    mock_skill.source_exists = True
    ...
    # Verify description includes category
    assert f"[{mock_skill.category}]" in call_kwargs["description"]
