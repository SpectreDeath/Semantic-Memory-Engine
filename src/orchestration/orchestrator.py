...
class PipelineJobQueue:
    ...
    def submit_job(
        self, job_id: str, job_type: str, payload: dict[str, Any], max_retries: int = 3
    ) -> dict[str, Any]:
    ...
    def get_pending_jobs(self, limit: int = 10) -> list[dict[str, Any]]:
        ...
        try:
            ...
            jobs = [
                {"job_id": r[0], "job_type": r[1], "payload": json.loads(r[2]) if r[2] else {}}
                for r in results
            ]
    ...
    def retry_job(self, job_id: str, delay_seconds: int = 60) -> dict[str, Any]:
        ...
        try:
            ...
            cursor.execute(
                "SELECT retry_count, max_retries FROM job_queue WHERE job_id = ?", (job_id,)
            )
            result = cursor.fetchone()
    ...
    def execute_pipeline(self, pipeline_name: str) -> dict[str, Any]:
        ...
        for job in pending:
            ...
            execution["step_results"].append(
                {"job_id": job["job_id"], "job_type": job["job_type"], "status": "processing"}
            )
