...
def scan_keywords(file_path, lines=20):
    keywords = {
        "pyod": "Data Forensic Scientist",
        "sklearn": "Data Forensic Scientist",
...
def get_persona(ext, specs):
    if ext == ".md":
        return "Technical Writer", "Documentation"
    if ext in [".csv", ".json"]:
        return "Data Auditor", "Data Analysis"
    if "Data Forensic Scientist" in specs:
        return "Data Forensic Scientist", "Forensic"
    if "Backend Architect" in specs:
        return "Backend Architect", "Backend"
    if "Data Engineering" in specs:
        return "Data Engineer", "Data Processing"
    if "ML Engineer" in specs:
        return "ML Engineer", "ML Engineering"
...
def update_json(persona, specialty):
    ...
    with open("active_persona.json", "w") as f:
