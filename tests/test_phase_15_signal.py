...
def test_forensic_signal():
    ...
    print(
        f"[+] DTW Alignment Cost: {res_sim['alignment_cost']} (Similarity: {res_sim['similarity_score']})"
    )
    ...
    print(
        f"[+] DTW Diff Cost: {res_diff['alignment_cost']} (Similarity: {res_diff['similarity_score']})"
    )
